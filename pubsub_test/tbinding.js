    ...
    return {
        ...
        'initModel': function(modelName) {
            ...
            Pubsub.subscrib('ui-update-event', function(FullProName, proVale) {
                var proName = FullProName.split('.')[1];
                self.updateModel(proName, proVale);
        ...
        'loadModel': function(modelData) {
            for (pro in modelData) {
                this.updateModel(pro, modelData[pro]);

            }

        },
        'updateModel': function(proName, proVale) {
            eval(this.modelName)[proName] = proVale;
            Pubsub.publish('model-update-event', this.modelName + "." + proName, proVale);
